...
///Furher Improvement Suggestions
//Place stoploss and takeprofit at technical levels - this can be done interactively through horizontal lines
//Dropdown list for BUY, SELL

extern int Slippage=5;
extern int Max_Spread_Pips=5;
extern double Balance_Risk_In_Percent = 1.0; 
input double Lots = 0; //Lot size. If 0 then lot size is calculated from Balance_Risk_In_Percent.  
...
void CheckProfit()
{
   if(OrderSelect(Tickets[0],SELECT_BY_TICKET) && OrderCloseTime() == 0)
   {
      if(PlaceStops)
      {
         if(Buy_Or_Sell=="Buy") {
            bool m1 = OrderModify(OrderTicket(),OrderOpenPrice(),OrderOpenPrice()-(Stop_Loss_In_Pips*UsePoint),OrderOpenPrice()+(Profit_In_Pips*UsePoint),0,Green); 
            if (m1) PlaceStops = false;
         }
         if(Buy_Or_Sell=="Sell") {
            bool m2 = OrderModify(OrderTicket(),OrderOpenPrice(),OrderOpenPrice()+(Stop_Loss_In_Pips*UsePoint),OrderOpenPrice()-(Profit_In_Pips*UsePoint),0,Red); 
            if (m2) PlaceStops = false;
         }
      ...
      if(BreakEven && ((Buy_Or_Sell=="Buy" && Bid>=OrderOpenPrice()+(Move_Stop_At_Pips*UsePoint)) || (Buy_Or_Sell=="Sell" && Ask<=OrderOpenPrice()-(Move_Stop_At_Pips*UsePoint)))) 
      {
         ...
         if(Buy_Or_Sell=="Buy") {
            bool m3 = OrderModify(OrderTicket(),OrderOpenPrice(),OrderOpenPrice()+(Move_Stop_To_Pips*UsePoint),OrderTakeProfit(),0,Green); 
            if (m3) BreakEven=false;
         }
         if(Buy_Or_Sell=="Sell") {
            bool m4 = OrderModify(OrderTicket(),OrderOpenPrice(),OrderOpenPrice()-(Move_Stop_To_Pips*UsePoint),OrderTakeProfit(),0,Red); 
            if (m4) BreakEven = false;
         }
      }
   }
return;
...
void OpenTrade()
{
   ...
   if(Buy_Or_Sell=="Buy")
   {
      ...
      Print ("Order placed. Ticket is: ", Tickets[0]);
      ...
      if(errorCode==4110) {
         ...
         BuysNotAllowed=true;
         Print ("Buys Not Allowed");
         return;
      }
      
      if (errorCode != 0) {
         Print ("Error placing order. Error code is: ", errorCode);
      }
